...
function db.createIncident(title, author, citizenid)
    ...
    createRecentActivity(citizenid, 'incidents', 'créé', response)
...
function db.deleteIncident(incidentId, citizenid)
    ...
    -- Add logs
    createRecentActivity(citizenid, 'incidents', 'suprimé', incidentId)
...
function db.saveCriminal(incidentId, criminal, officerCitizenid)
    createRecentActivity(officerCitizenid, 'incidents', 'mis à jour', incidentId)
    local queries = {
