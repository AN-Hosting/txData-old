CREATE TABLE
  IF NOT EXISTS `mdt_offenses` (
    ...
    `type` ENUM('délit', 'crime', 'contravention') NOT NULL,
    `category` ENUM(
      ...
      'INFRACTIONS LIÉES À LA CONDUITE D UN VÉHICULE',
      'INFRACTIONS CONTRE LE BIEN-ÊTRE DE LA FAUNE') NOT NULL,
...
CREATE TABLE
  IF NOT EXISTS `mdt_recent_activity` (
    ...
    `type` ENUM('créé', 'mis à jour', 'suprimé') NOT NULL,
    `date` datetime DEFAULT CURRENT_TIMESTAMP,
