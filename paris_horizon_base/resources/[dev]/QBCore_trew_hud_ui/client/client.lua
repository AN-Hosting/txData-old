...
AddEventHandler('playerLoaded', function()
	...
	if Config.ui.toggleui then
		if (Config.ui.showJob == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'job' })
		end
		if (Config.ui.showGang == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'gang' })
		end
...
Citizen.CreateThread(function()
	...
	while Config.ui.showLocation do
		...
		locationMessage = string.format(
			Config.Locale['you_are_on_location'],
			locationMessage
		)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Player.job then
    	    ...
    	    if Player.job.label == Player.job.grade.name then
    	      	job = Player.job.grade.name
    	    else
    	       	job = Player.job.label .. ': ' ..  Player.job.grade.name
    	    end

			if Player.gang.label == Player.gang.grade.name then
				gang = Player.gang.grade.name
		  else
			gang = Player.gang.label .. ': ' ..  Player.gang.grade.name
		  end

			blackMoney = Player.money.crypto
			bank = Player.money.bank
			money = Player.money.cash

  			SendNUIMessage({ action = 'setText', id = 'job', value = job })
			SendNUIMessage({ action = 'setText', id = 'gang', value = gang })
...
function trewDate()
	...
	local date_format = Config.Locale['date_format'][Config.date.format]
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'gang' })
		SendNUIMessage({ action = 'element', task = 'disable', value = 'society' })
