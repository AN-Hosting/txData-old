Bones = {Options = {}, Vehicle = {'chassis', 'windscreen', 'seat_pside_r', 'seat_dside_r', 'bodyshell', 'suspension_lm', 'suspension_lr', 'platelight', 'attach_female', 'attach_male', 'bonnet', 'boot', 'chassis_dummy', 'chassis_Control', 'door_dside_f', 'door_dside_r', 'door_pside_f', 'door_pside_r', 'Gun_GripR', 'windscreen_f', 'platelight', 'VFX_Emitter', 'window_lf', 'window_lr', 'window_rf', 'window_rr', 'engine', 'gun_ammo', 'ROPE_ATTATCH', 'wheel_lf', 'wheel_lr', 'wheel_rf', 'wheel_rr', 'exhaust', 'overheat', 'seat_dside_f', 'seat_pside_f', 'Gun_Nuzzle', 'seat_r'}}

if Config.EnableDefaultOptions then
    local BackEngineVehicles = {
        [`ninef`] = true,
        [`adder`] = true,
        [`vagner`] = true,
        [`t20`] = true,
        [`infernus`] = true,
        [`zentorno`] = true,
        [`reaper`] = true,
        [`comet2`] = true,
        [`comet3`] = true,
        [`jester`] = true,
        [`jester2`] = true,
        [`cheetah`] = true,
        [`cheetah2`] = true,
        [`prototipo`] = true,
        [`turismor`] = true,
        [`pfister811`] = true,
        [`ardent`] = true,
        [`nero`] = true,
        [`nero2`] = true,
        [`tempesta`] = true,
        [`vacca`] = true,
        [`bullet`] = true,
        [`osiris`] = true,
        [`entityxf`] = true,
        [`turismo2`] = true,
        [`fmj`] = true,
        [`re7b`] = true,
        [`tyrus`] = true,
        [`italigtb`] = true,
        [`penetrator`] = true,
        [`monroe`] = true,
        [`ninef2`] = true,
        [`stingergt`] = true,
        [`surfer`] = true,
        [`surfer2`] = true,
        [`gp1`] = true,
        [`autarch`] = true,
        [`tyrant`] = true
    }

    local function CleanupTargetInteraction()
        -- Réinitialiser l'état du target après l'interaction
        SetNuiFocus(false, false)
        ClearPedTasks(PlayerPedId())
    end

    local function ToggleDoor(vehicle, door)
        if GetVehicleDoorLockStatus(vehicle) < 2 then
            if GetVehicleDoorAngleRatio(vehicle, door) > 0.0 then
                SetVehicleDoorShut(vehicle, door, false)
            else
                SetVehicleDoorOpen(vehicle, door, false)
            end
        end
        CleanupTargetInteraction()
    end

    local function CheckPlayerInVehicle()
        local playerId = PlayerId()
        local playerPed = GetPlayerPed(playerId)
        if IsPedInAnyVehicle(playerPed, false) then
            return true
        else
            return false
        end
    end

    -- Define all options for the vehicle
    local VehicleOptions = {
        ["seats"] = {
            label = 'Sièges',
            icon = 'fas fa-chair',
            canInteract = function()
                return CheckPlayerInVehicle()
            end,
            subOptions = {
                {
                    id = 'seat0',
                    label = 'Conducteur',
                    icon = 'fas fa-chair',
                    canInteract = function(entity)
                        return  IsVehicleSeatFree(entity, -1)
                    end,    
                    action = function(entity)
                        SetPedIntoVehicle(PlayerPedId(), entity, -1)
                        CleanupTargetInteraction()
                    end,
                },
                {
                    id = 'seat1',
                    label = 'Passager',
                    icon = 'fas fa-chair',
                    canInteract = function(entity)
                        return  IsVehicleSeatFree(entity, 0)
                    end,
                    action = function(entity)
                        SetPedIntoVehicle(PlayerPedId(), entity, 0)
                        CleanupTargetInteraction()
                    end,
                },
                {
                    id = 'seat2',
                    label = 'Arrière gauche',
                    icon = 'fas fa-chair',
                    canInteract = function(entity)
                        return  IsVehicleSeatFree(entity, 1)
                    end,
                    action = function(entity)
                        SetPedIntoVehicle(PlayerPedId(), entity, 1)
                        CleanupTargetInteraction()
                    end,
                },
                {
                    id = 'seat3',
                    label = 'Arrière droite',
                    icon = 'fas fa-chair',
                    canInteract = function(entity)
                        return  IsVehicleSeatFree(entity, 2)
                    end,
                    action = function(entity)
                        SetPedIntoVehicle(PlayerPedId(), entity, 2)
                        CleanupTargetInteraction()
                    end,
                },
            },
        },
        ['doors'] = {           
            label = 'Portes',
            icon = 'fas fa-door-open',
            subOptions = {
                {
                    id = 'door4',
                    label = 'Capot',
                    icon = 'car',
                    action = function(entity)
                    ToggleDoor(entity, BackEngineVehicles[GetEntityModel(entity)] and 5 or 4)
                    end,
                }, {
                    id = 'door0',
                    label = 'Conducteur',
                    icon = 'car-side',
                    canInteract = function(entity)
                    return GetEntityBoneIndexByName(entity, 'door_dside_f') ~= -1
                    end,
                    action = function(entity)
                        ToggleDoor(entity, 0)
                    end,
                }, {
                    id = 'door1',
                    label = 'Passager',
                    icon = 'car-side',
                    canInteract = function(entity)
                    return GetEntityBoneIndexByName(entity, 'door_pside_f') ~= -1
                    end,
                    action = function(entity)
                        ToggleDoor(entity, 1)
                    end,
                }, {
                    id = 'door3',
                    label = 'Arrière gauche',
                    icon = 'car-side',
                    canInteract = function(entity)
                    return GetEntityBoneIndexByName(entity, 'door_dside_r') ~= -1
                    end,
                    action = function(entity)
                        ToggleDoor(entity, 2)
                    end,
                }, {
                    id = 'door2',
                    label = 'Arrière droite',
                    icon = 'car-side',
                    canInteract = function(entity)
                    return GetEntityBoneIndexByName(entity, 'door_pside_r') ~= -1
                    end,
                    action = function(entity)
                        ToggleDoor(entity, 3)
                    end,
                }, {
                    id = 'door5',
                    label = 'Coffre',
                    icon = 'car',
                    action = function(entity)
                    ToggleDoor(entity, BackEngineVehicles[GetEntityModel(entity)] and 4 or 5)
                    end,
                },
            },
        }
    }

    -- Assign all options to the vehicle context menu
    for _, bone in ipairs(Bones.Vehicle) do
        Bones.Options[bone] = VehicleOptions
    end
end

return Bones