...
RegisterNetEvent('kezi:moneywash', function(amount, playerCoords)
    ...
    local tax = math.random(10, 30)--tonumber(currentLocation.tax)
    local bidentax = (amount * tax) / 100
    local newAmount = amount - bidentax

    if Player.Functions.RemoveItem('black', amount) then
        TriggerClientEvent('QBCore:Notify', src, 'Tu a donné' .. amount .. '€ d\'argent sale et recu ' .. tonumber(newAmount) .. '€ avec une taxe de ' .. tax .. '%', 'success')
