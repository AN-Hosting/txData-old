function openwasher()
    local input = lib.inputDialog('Blanchiment d\'Argent', {'Argent Sale'})
        
    if not input then return end

    if tonumber(input[1]) == 0 then 
         lib.notify({
             title = 'Blanchiment | Erreur',
            description = 'La valeur ne peut pas être 0',
           type = 'info',
             position = 'top'
         })
        return
...
function point:onEnter()
    lib.showTextUI('[E] - Blanchir de l\'argent')
end 
...
RegisterKeyMapping('washmoney', 'Blanchir de l\'argent', 'keyboard', 'E') 
