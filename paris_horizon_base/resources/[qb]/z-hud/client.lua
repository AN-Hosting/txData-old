...
CreateThread(function()
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            local jobLabel = Player.job.label .. " - " .. Player.job.grade.name
            local gangLabel = Player.gang.label
            if Player.gang.grade then
                gangLabel = Player.gang.label .. " - " .. Player.gang.grade.name
            end
            
            -- Ajouter des vérifications pour s'assurer que les valeurs existent
...
-- Modifier l'event de mise à jour d'argent
...
-- Mettre à jour quand le job change
RegisterNetEvent('QBCore:Client:OnJobUpdate')
AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo)
    local jobLabel = JobInfo.label .. " - " .. JobInfo.grade.name
    SendNUIMessage({
        action = 'updateStatus',
        job = jobLabel
    })
...
-- Mettre à jour quand le gang change
RegisterNetEvent('QBCore:Client:OnGangUpdate')
AddEventHandler('QBCore:Client:OnGangUpdate', function(GangInfo)
    local gangLabel = GangInfo.label
    if GangInfo.grade then
        gangLabel = GangInfo.label .. " - " .. GangInfo.grade.name
    end
    SendNUIMessage({
        action = 'updateStatus',
        gang = gangLabel
...
-- Modifier la fonction isWeaponUsingAmmo pour mieux détecter les armes
local function isWeaponUsingAmmo(weapon)
    if weapon == nil or weapon == 0 or weapon == -1569615261 then -- -1569615261 est le hash pour les poings
        return false
    end
    local weaponGroup = GetWeapontypeGroup(weapon)
    return weaponGroup ~= 0 and weaponGroup ~= 2685387236 -- 2685387236 est le hash pour le groupe melee
end

-- Modifier la thread de mise à jour des munitions
CreateThread(function()
    while true do
        if LocalPlayer.state.isLoggedIn then
            local player = PlayerPedId()
            ...
            if currentWeapon and isWeaponUsingAmmo(currentWeapon) then
                currentAmmo = GetAmmoInPedWeapon(player, currentWeapon)
                maxAmmo = GetMaxAmmoInClip(player, currentWeapon, true)
