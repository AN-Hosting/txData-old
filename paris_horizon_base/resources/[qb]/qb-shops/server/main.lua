...
-- Ajouter cet événement pour gérer les achats
RegisterNetEvent('qb-shops:server:buyItem', function(items)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end

    local totalCost = 0
    -- Calculer d'abord le coût total
    for itemName, itemData in pairs(items) do
        totalCost = totalCost + (itemData.price * itemData.amount)
    end

    -- Vérifier si le joueur a assez d'argent
    if Player.Functions.GetMoney('cash') >= totalCost then
        -- Retirer l'argent d'abord
        if Player.Functions.RemoveMoney('cash', totalCost, 'shop-items') then
            -- Puis ajouter les items
            local success = true
            for itemName, itemData in pairs(items) do
                -- Vérifier si l'item peut être ajouté
                if not Player.Functions.AddItem(itemName, itemData.amount) then
                    success = false
                    break
                end
            end
            ...
            if success then
                TriggerClientEvent('QBCore:Notify', src, 'Achat effectué avec succès!', 'success')
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], "add")
            else
                -- Si l'ajout d'items échoue, rembourser le joueur
                Player.Functions.AddMoney('cash', totalCost, 'shop-items-refund')
                TriggerClientEvent('QBCore:Notify', src, 'Erreur: Inventaire plein!', 'error')
            end
        end
    else
...
-- Dans la fonction SetupShopItems, ajouter la gestion des distributeurs
RegisterNetEvent('qb-shops:server:SetupShopItems', function(shopType)
    -- ... code existant ...
    
    if shopType == "distributeur" then
        ShopItems[shopType] = {
            label = Config.Shops[shopType].label,
            items = Config.Shops[shopType].products,
            slots = #Config.Shops[shopType].products
        }
    end
