	...
	switch (event.data.action) {
			...
			if (selectedWindow == "none") {
				$('#menu').html(`
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						<br>
						<div class="ldata-txt">Chargement des donnees...</div>
					</div>
				`);
				$("#menu").fadeIn();
				$(".main_card").fadeIn();
				...
				setTimeout(function(){
					...
					$('#menu').html(`
						<div class="col-md-2 d-flex flex-column sidebar-s">
							...
							<hr>
							<span class="sidebar-title">Personnel</span>
							<div id="sidebar"></div>
							<p class="sidebar-item mt-auto logout"><i class="fas fa-sign-out-alt"></i></i> <span class="ms-1">Deconnexion</span></p>
						</div>
						<div class="col-md-10 tab-s">
							<div class="d-flex justify-content-between align-items-center">
								<span class="selected-page"><span id="page-title">Aperçu</span></span>
								<div>
									<span class="username align-middle">
										<span id="playerName"></span> <span id="avatar">${avatar}</span>
									</span>
									<div class="wallet-div">
										<span>Portefeuille: <span id="wallet_money"></span> EUR</span>
									</div>
								</div>
							</div>
							<hr>
		...
		case 'transactions_page':
			...
			if (event.data.isInSociety){
				society = `<span class="sidebar-title mt-5">Societe</span>
						   <p class="sidebar-item mt-2" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Apercu</span></p>
			...
			$('#sidebar').html(`
				<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Apercu</span></p>
				<p class="sidebar-item selected" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transactions</span></p>
				<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Parametres</span></p>
				${society}
			`);
			...
			for(var i = 0; i < num; i++) {
				...
				// Sent
					...
					data = `<td class="align-middle fw500">
								A <span class="transactions-name">${name}</span>
								<div class="mtm3125">Envoye</div>
							</td>`;
				...
				// Deposited
					...
					icon = '<td class="align-middle"><span class="transactions-action"><i class="bi bi-download"></i></span></td>';
					data = `<td class="align-middle fw500">
								Dans <span class="transactions-name">${name}</span>
								<div class="mtm3125">Depose</div>
							</td>`;
					amount = `<td class="align-middle fw500 transactions-received text-center">+ ${db.value.toLocaleString()} EUR</td>`;
				// Withdrawn
			...
			$('#page_info').html(`
				<div class="row mb-3">
					...
					<div class="col col-md-3 pr05 pl05">
						<div class="card stats-title">
							<div class="card-body text-center">
								<h6 class="card-title">DEPENSES</h6>
...
function overview_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "bankmenu" || !event.data.isUpdate){
		...
		$('#sidebar').html(`
			<p class="sidebar-item mt-2 selected" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Apercu</span></p>
			<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transactions</span></p>
			<p class="sidebar-item mt-2" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Parametres</span></p>
			${society}
		`);
		...
		if(event.data.RequireCC) {
			informations = `<span class="card-o-title">Informations<span class="badge bg-primary viewall-badge buy_new_card" data-bs-toggle="modal" data-bs-target="#buycc_modal"><i class="fa-solid fa-plus"></i> NOUVEAU</span></span>`;
		} else {
			...
			<div class="col-md-7 chart-main">
				<div class="card chart-card text-center">
					<div class="card-header card-o-header">
						<span class="card-o-title">Statistiques</span>
					</div>
					...
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Dernieres Transactions<span class="badge bg-primary viewall-badge" id="view_all_transactions"><i class="fas fa-eye"></i> VOIR TOUT</span></span>
							</div>
						...
						<div class="card-body ccard-body">
							<div class="card creditcard-classic_card d-flex align-items-center">
								<div class="card-body creditcard-classic_card-body">
									<span class="d-flex justify-content-between"><span><img src="img/visa_white.svg" class="w20p"><span class="ccard-name">okokBank Classique</span></span><span><i class="fas fa-wifi"></i></span></span>
									<div class="mt38p">
										<div class="d-flex align-items-center">
											<span class="ccard-status">Statut</span>
										</div>
										<div class="d-flex justify-content-between align-items-center">
											<span class="ccard-active">ACTIF</span>
											<div class="d-flex align-items-center ccard-valid">
												<span class="ccard-thru">VALIDE JUSQU'A</span>
							...
							<hr>
							<p class="card-text text-center ccard-fs" style="margin-bottom: 1.1rem;"><span class="fff">Solde:</span> <span id="playerBankMoney"></span> EUR</p>
					...
					<div class="card text-center h-100 actions-card">
						...
						<div class="card-body actions-card_body">
							<div class="d-flex justify-content-center">
								<button type="button" id="depositMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#depositModal"><i class="bi bi-upload"></i> Deposer</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
								<button type="button" id="withdrawMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#withdrawModal"><i class="bi bi-download"></i> Retirer</button>
							</div>
							<div class="d-flex justify-content-center mt4375">
								<button type="button" id="transferMoneyModal" class="btn btn-blue flex-grow-1" data-bs-toggle="modal" data-bs-target="#transferModal"><i class="fas fa-exchange-alt"></i> Virement</button>
							</div>
						</div>
					</div>
				</div>
		...
		for(var i = 0; i < num; i++) {
			...
			// Sent
				...
				data = `<td class="align-middle lastT-name-div">
							A <span class="lastT-name">${name}</span>
							<div class="mtm3125">Envoye</div>
						</td>`;
			...
			// Deposited
				...
				icon = '<td class="align-middle"><span class="lastT-action"><i class="bi bi-download"></i></span></td>';
				data = `<td class="align-middle lastT-name-div">
							Dans <span class="lastT-name">${name}</span>
							<div class="mtm3125">Depose</div>
						</td>`;
				amount = `<td class="align-middle fw500 lastT-received"><span class="floatr">+ ${db.value.toLocaleString()} EUR</span></td>`;
			// Withdrawn
...
function society_page_function(event) {
	if(event.data.isUpdate && selectedWindow == "societies" || !event.data.isUpdate){
		...
		if (event.data.isInSociety){
			society = `<span class="sidebar-title mt-5">Societe</span>
					   <p class="sidebar-item mt-2 selected" id="society_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Apercu</span></p>
					   <p class="sidebar-item" id="society_transactions"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transactions</span></p>`;
		} else {
			society = '';
		}
			...
			<div class="col-md-7 chart-main">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="card last-t-card">
							<div class="card-header card-o-header text-center">
								<span class="card-o-title">Dernieres Transactions<span class="badge bg-primary viewall-badge" id="view_all_transactions_society"><i class="fas fa-eye"></i> VOIR TOUT</span></span>
							</div>
							<div class="card-body" id="last-t-body">
								<table id="lastTransactionsTable">
									<tbody id="lastTransactionsData">
...
function settings_page_function(event) {
	...
	$('#sidebar').html(`
		<p class="sidebar-item mt-2" id="overview_page"><i class="bi bi-grid-1x2-fill"></i> <span class="ms-1">Apercu</span></p>
		<p class="sidebar-item" id="transactions_page"><i class="fas fa-exchange-alt"></i> <span class="ms-1">Transactions</span></p>
		<p class="sidebar-item mt-2 selected" id="settings_page"><i class="fas fa-cog"></i> <span class="ms-1">Parametres</span></p>
		${society}
	`);
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changeiban-card">
									<div class="card-body text-center">
										<span class="fs15 fff">Changer IBAN</span>
										<input type="text" maxlength="${event.data.ibanCharNum}" id="new_iban" class="form-control text-center" placeholder="Nouvel IBAN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_iban" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Soumettre</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Le IBAN a un cout de changement de ${event.data.ibanCost}€</span>
										<hr class="fff">
										<span>Le IBAN a toujours le prefixe "${event.data.ibanPrefix}"</span>
										<hr class="fff">
										<span>Le nombre maximum de caracteres est ${event.data.ibanCharNum}</span>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col col-md-6 d-flex justify-content-center">
								<div class="card w-100 h-100 changepin-card">
									<div class="card-body text-center">
										<span class="fs15 fff">Changer PIN</span>
										<input type="password" maxlength="4" id="new_pin" class="form-control text-center" placeholder="Nouveau PIN" onkeyup="checkIfEmptySettings()">
										<button type="button" id="change_pin" class="btn btn-blue" disabled><i class="fas fa-paper-plane"></i> Soumettre</button>
									</div>
								</div>
							</div>
							<div class="col col-md-6 d-flex align-items-center">
								<div class="card w-100 h-100 d-flex justify-content-center settings_info-card">
									<div class="card-body text-center fs1125">
										<span>Le PIN a un cout de changement de ${event.data.pinCost}€</span>
										<hr class="fff">
										<span>Le nombre maximum de caracteres est ${event.data.pinCharNum}</span>
										<hr class="fff">
	...
	setTimeout(function () {
		$('#menu').html(`
			<div class="d-flex justify-content-center flex-column align-items-center">
				<span class="load"></span>
				<br>
				<div class="ldata-txt">Deconnexion...</div>
			</div>
		`);
		$("#menu").fadeIn();
		setTimeout(function(){
