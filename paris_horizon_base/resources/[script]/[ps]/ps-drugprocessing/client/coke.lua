...
RegisterNetEvent('ps-drugprocessing:pickCocaLeaves', function()
    ...
    QBCore.Functions.TriggerCallback('ps-drugprocessing:validate_items', function(result)
        ...
        -- Si on arrive ici, le joueur a les ciseaux
        ...
        trimming_scissors = 1  -- Vérifie si le joueur a au moins 1 ciseau
    })
end)
...
RegisterNetEvent('ps-drugprocessing:client:cutCokeBrick', function()
    local playerPed = PlayerPedId()
    
    -- Animation de découpe
    TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
    QBCore.Functions.Progressbar("cut_coke_brick", "Découpe de la brick...", 7500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('ps-drugprocessing:cutCokeBrick')
        ClearPedTasks(playerPed)
    end, function() -- Cancel
        ClearPedTasks(playerPed)
    end)
