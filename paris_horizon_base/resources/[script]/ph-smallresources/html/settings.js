...
function initSettings() {
  ...
  const container = $('.notif-settings-container');
  if (container.length) {
    container.css({
  ...
  setTimeout(setHorizontalButton, 0)
...
function setHorizontalButton() {
  const container = $('.notif-settings-container');
  const leftPosition = container.css('left');
  
  if (!leftPosition) return;
  
  if (parseInt(leftPosition.replace('px', '')) < window.innerWidth / 2) {
