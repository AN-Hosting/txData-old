...
local eModelsWithNoSrn = -- Pas de sirene
{
	-- Vinci
	'Fs_cliovinci',
	'Fs_mastervinci',
...
function useNoSiren(veh)
	local model = GetEntityModel(veh)
	for i = 1, #eModelsWithNoSrn, 1 do
		if model == GetHashKey(eModelsWithNoSrn[i]) then
			return true
		end
	end
	return false
...
function SetLxSirenStateForVeh(veh, newstate)
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate ~= state_lxsiren[veh] then
			...
			if useEMSSiren(veh) then
				snd_lxsiren[veh] = GetSoundId()
				PlaySoundFromEntity(snd_lxsiren[veh], "SIREN_HOTEL", veh, "DLC_WMSIRENS_SOUNDSET", 0, 0)
				TogMuteDfltSrnForVeh(veh, false)
			else
				snd_lxsiren[veh] = GetSoundId()
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, 80) or IsDisabledControlJustReleased(0, 81) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										if not useNoSiren(veh) and not useBrigadeSiren(veh) and not useBrigadeTruckSiren(veh) then
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
										if cstate == 1 then
											nstate = 2
