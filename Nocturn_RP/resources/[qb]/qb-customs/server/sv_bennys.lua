...
RegisterNetEvent('qb-customs:server:attemptPurchase', function(type, upgradeLevel)
	...
	exports['okokBanking']:AddMoney("mechanic", repairCost)
        else
	...
	exports['okokBanking']:AddMoney("mechanic", vehicleCustomisationPrices[type].prices[upgradeLevel])
        else
            TriggerClientEvent('qb-customs:client:purchaseFailed', source)
        end
    else
	...
	exports['okokBanking']:AddMoney("mechanic", vehicleCustomisationPrices[type].price)
        else
            TriggerClientEvent('qb-customs:client:purchaseFailed', source)
        end
    end
...
RegisterNetEvent('qb-customs:server:UpdateLocation', function(location, type, key, value)
    local source = source
    if not QBCore.Functions.HasPermission(source, 'admin') then return CancelEvent() end
