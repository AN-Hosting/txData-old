...
local function IncreasePlayerXP(source, xpGain, xpType)
    ...
    if Player then
        ...
        TriggerClientEvent('QBCore:Notify', source, string.format('Vous avez gagné %d XP en %s', xpGain, xpType), 'success')
    end
end
...
RegisterNetEvent('qb-crafting:server:removeCraftingTable', function(benchType)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Table de craft retirée', 'success')
end)
...
RegisterNetEvent('qb-crafting:server:receiveItem', function(craftedItem, requiredItems, amountToCraft, xpGain, xpType)
    ...
    if canGive then
        ...
        TriggerClientEvent('QBCore:Notify', src, string.format('Vous avez fabriqué %dx %s', amountToCraft, QBCore.Shared.Items[craftedItem].label), 'success')
        if xpGain and xpType then
            IncreasePlayerXP(src, xpGain, xpType)
        end
    end
end)

-- Initialisation des métadonnées XP
RegisterNetEvent('QBCore:Server:OnPlayerLoaded', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        -- Initialiser craftingrep si n'existe pas
        if not Player.PlayerData.metadata['craftingrep'] then
            Player.Functions.SetMetaData('craftingrep', 0)
        end
        -- Initialiser attachmentcraftingrep si n'existe pas
        if not Player.PlayerData.metadata['attachmentcraftingrep'] then
