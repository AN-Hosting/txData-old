...
local function Hotwire(vehicle, plate)
    ...
    SetVehicleAlarmTimeLeft(vehicle, Config.minHotwireTime)
    ...
    local numChecks = math.random(2, 4)
    local checks = {}
    
    -- Génération aléatoire des checks
    for i = 1, numChecks do
        local difficultyIndex = math.random(1, #difficulties)
        checks[i] = difficulties[difficultyIndex]
    end
    ...
    if success then
        ...
        Wait(1000)
        AttemptPoliceAlert("steal")
        ShowNotification("Vous avez réussi", "success")
    else
        ShowNotification(Lang:t("message.hotwiring_fail"), "error")
        SetVehicleEngineOn(vehicle, false, false, true)
        IsHotwiring = false
        -- Réinitialiser HwText immédiatement pour que le texte 3D réapparaisse
        HwText = true
