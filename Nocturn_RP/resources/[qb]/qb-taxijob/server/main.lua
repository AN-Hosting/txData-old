...
RegisterNetEvent('qb-taxi:server:NpcPay', function(payment, hasReceivedBonus)
    ...
    if Player.PlayerData.job.name == Config.jobRequired then
        if NearTaxi(src) then
            ...
            if chance < 26 then
                exports['qs-inventory']:AddItem(src, Config.Rewards, 1, false, false, 'qb-taxi:server:NpcPay')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.Rewards], 'add')
            end
        else
