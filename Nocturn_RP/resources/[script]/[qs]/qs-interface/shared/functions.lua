...
function Warning(...)
    local msg = '^3[INTERFACE WARNING]:^0 '
    for i, v in pairs({ ... }) do
        msg = msg .. tostring(v) .. '\t'
    end
    msg = msg
...
---@return vector4
function GetEntityCoordsFromOffset(vehicleCoords, offset)
    local xOffset = offset.x
    local yOffset = offset.y
    local x = vehicleCoords.x + (xOffset * math.cos(math.rad(vehicleCoords.w + 90))) - (yOffset * math.sin(math.rad(vehicleCoords.w + 90)))
    local y = vehicleCoords.y + (xOffset * math.sin(math.rad(vehicleCoords.w + 90))) + (yOffset * math.cos(math.rad(vehicleCoords.w + 90)))
    local z = vehicleCoords.z + offset.z
    return vec4(x, y, z, vehicleCoords.w)
end
