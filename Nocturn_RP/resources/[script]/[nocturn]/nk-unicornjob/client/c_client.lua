...
RegisterNetEvent('nk-unicornjob:Client:Notify')
AddEventHandler("nk-unicornjob:Client:Notify", function(msg,type)
    Notify(msg,type)
end)

AddEventHandler("nk-unicornjob:Client:Storage", function()
...
AddEventHandler("nk-unicornjob:Client:OpenTray01", function()
    TriggerEvent(Config.Stash.StashInvTrigger, "Tray1")
    TriggerServerEvent(Config.Stash.OpenInvTrigger, "stash", "Tray1", {
        maxweight = 500,
        slots = 5,
    })
end)

AddEventHandler("nk-unicornjob:Client:OpenTray02", function()
    TriggerEvent(Config.Stash.StashInvTrigger, "Tray2")
    TriggerServerEvent(Config.Stash.OpenInvTrigger, "stash", "Tray2", {
        maxweight = 500,
        slots = 5,
    })
end)
...
RegisterNetEvent("nk-unicornjob:Client:OpenShop", function(index)
...
AddEventHandler('nk-unicornjob:Client:Sentar', function()
    TriggerEvent('animations:client:EmoteCommandStart', {"sitchair"})
end)

-- || ===============> Faturar
 
RegisterNetEvent('nk-unicornjob:Client:Faturamento', function()
    if Config.JimPayments then
        ...
        if dialog then
            ...
            TriggerServerEvent("nk-unicornjob:Server:Billing", dialog.id, dialog.amount)
        end
    end
end)
...
function CraftNonAlcoholicMenu()
    ...
    for k, v in pairs(Config.NonAlcoholic) do
        ...
        item.text = text
        item.params = { event = 'nk-unicornjob:client:CraftNonAlcoholic', args = { type = k } }
...
function CraftNonAlcoholic(weapon)
    QBCore.Functions.Progressbar('CraftNonAlcoholic', Language.Progressbars.Make..Config.NonAlcoholic[weapon].label, 5000, false, false, {
        ...
        TriggerServerEvent('nk-unicornjob:server:CraftNonAlcoholic', Config.NonAlcoholic[weapon].hash)
        for k, v in pairs(Config.NonAlcoholic[weapon].materials) do
             TriggerServerEvent('nk-unicornjob:Server:RemoveItem', v.item, v.amount)
...
RegisterNetEvent('nk-unicornjob:client:CraftNonAlcoholic', function(data)
    QBCore.Functions.TriggerCallback("nk-unicornjob:server:Materials", function(hasMaterials)
...
function CraftAlcoholicMenu()
    ...
    for k, v in pairs(Config.Alcoholic) do
        ...
        item.text = text
        item.params = { event = 'nk-unicornjob:client:CraftAlcoholic', args = { type = k } }
        table.insert(columns, item)
    end
    exports['qb-menu']:openMenu(columns)
end
...
function CraftAlcoholic(weapon)
    QBCore.Functions.Progressbar('CraftAlcoholic', Language.Progressbars.Make..Config.Alcoholic[weapon].label, 5000, false, false, {
        ...
        TriggerServerEvent('nk-unicornjob:server:CraftAlcoholic', Config.Alcoholic[weapon].hash)
...
RegisterNetEvent('nk-unicornjob:client:CraftAlcoholic', function(data)
    QBCore.Functions.TriggerCallback("nk-unicornjob:server:Materials", function(hasMaterials)
        if (hasMaterials) then
...
RegisterNetEvent('nk-unicornjob:Client:GiveCoffee', function()
    ...
    Wait(5000)
    TriggerServerEvent('nk-unicornjob:Server:GiveCoffee')
end)
