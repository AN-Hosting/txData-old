    ...
    if properties["plateIndex"] ~= newproperties["plateIndex"] then
        for k, v in pairs(Loc[Config.Lan].vehiclePlateOptions) do
            if newproperties["plateIndex"] == v.id then
                vehlist[#vehlist+1] = locale("checkDetails", "wrapLabel").." - [ "..v.name.." ]"
                modlist[#modlist+1] = {
                    ["type"] = "("..v.name..")",
                    ["item"] = locale("checkDetails", "wrapLabel").." - "..Items["customplate"].label
                }
            end
        end
    end
    ...
    if Config.Previews.PreviewPhone and hasPhone then
        if vehlist[1] then
            ...
            local mailData = {
                ...
                message = veh.."<br>"..locale("checkDetails", "wrapLabel")..": "..vehplate
