...
MechanicTools.Menu = function(skip)
    ...
    then
        ...
        mechjobTable[#mechjobTable + 1] = {
            ...
            header = locale("checkDetails", "wheelsLabel"),
    ...
    for _, v in pairs(mechjobTable) do
        ...
        if (Config.Repairs.StashRepair or Config.Repairs.FreeRepair) and not skip then
            ...
            if not skip and not hasItem(costTable[v.part].cost) then
                ...
                if Config.System.Menu == "qb" then
                    costTable[v.part].txt = "<span style='color:red'>"..(costTable[v.part].txt or damageTable["wheels"]).."</span>"
                end
            end
        end
        ...
        if v.part == "wheels" then
            ...
            if not Config.Repairs.FreeRepair then
                RepairMenu[#RepairMenu].txt = locale("repairActions", "costLabel")..": "..damageTable["wheels"].." "..Items["sparetire"].label
            end
        end
    end
...
MechanicTools.Apply = function(data)
    ...
    local repairTable = {
        ...
        ["wheels"]  = { dict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", bartext = locale("repairActions", "actionChanging") },
    }
    ...
    if (Config.Repairs.StashRepair and checkStashItem(stashName, data.cost))
       or not Config.Repairs.StashRepair
    then
