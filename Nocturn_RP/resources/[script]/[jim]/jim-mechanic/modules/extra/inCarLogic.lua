...
RegisterNetEvent(getScript()..":Client:ExitVehicle", function()
    ...
    if not failedToLeave then
        ...
        if seat == -1 then
            if DoesEntityExist(veh) and Config.Overrides.saveOnExit == true and not PrevFunc.getInPreview() then
                local state = Entity(veh).state
                if state.canUpdate == nil or state.canUpdate == true then
                ...
                debugPrint("Vehicle was deleted, we'll assume a garage saved the mods")
            end
...
RegisterNetEvent(getScript()..":Client:EnteredVehicle", function()
    ...
    debugPrint("^5Debug^7: ^3EnteredVehicle^7: ^2Player has entered vehicle ^7- ^4Seat^7: ^1"..seat.."^7")
    ...
    CreateThread(function()
        ...
        CreateThread(function()
            if Config.vehFailure.PreventRoll and Config.vehFailure.oldPreventRoll and DoesEntityExist(veh) then
                while vehIsIn == veh do
                    if seat == -1 then
...
-- Damage Detection (moved from loop to enetiyDamaged event detection)
AddEventHandler('entityDamaged', function (entity, _, weapon, _)
    ...
    if frameBodyChange ~= 0 and frameBodyChange > 0 and currentVehCache.seat == -1 then
        ...
        if Config.vehFailure.increaseEngDamage then
            ...
            local extra  = frameBodyChange / random
            extra = (extra >= 1) and 0 or extra
            local engineDamage = extra > 0 and (extra * 100) or 0

            debugPrint("Applying extra engine damage: "..engineDamage, GetVehicleEngineHealth(currentVehCache.currentVeh) - engineDamage)
            SetVehicleEngineHealth(currentVehCache.currentVeh, GetVehicleEngineHealth(currentVehCache.currentVeh) - engineDamage)

            Wait(10)
            debugPrint("Applying extra body damage: "..extra, GetVehicleBodyHealth(currentVehCache.currentVeh) - extra)
            SetVehicleBodyHealth(currentVehCache.currentVeh, GetVehicleBodyHealth(currentVehCache.currentVeh) - extra)
        end
                ...
                if not inProgress then
                    ...
                    if VehicleStatus[plate].harness == 1 then
                        ...
                        if (Config.Harness.progOn and not harnessOn) or (Config.Harness.progOff and harnessOn) then
                            ...
                            if progressBar({
                                label = (not harnessOn and locale("common", "attachingHarness") or locale("common", "actionRemoving")..": "),
                                time  = time,
                                cancel= true,
                                anim  = "fixing_a_ped",
                                dict  = "mini@repair",
