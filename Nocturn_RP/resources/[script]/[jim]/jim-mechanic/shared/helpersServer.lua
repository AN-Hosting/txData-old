...
HelperServer.updateVehicleDatabase = function(props, plate)
	props.plate = plate
	if HelperServer.isVehicleOwned(plate) then
		debugPrint("^5SQL7: ^3updateVehicle^7: [^3"..props.model.."^7] - [^3"..plate.."^7]")
		...
		elseif isStarted(QBExport) or isStarted(QBXExport) then
			MySQL.Async.execute("UPDATE "..vehDatabase.." SET mods = ?, engine = ?, body = ?, fuel = ? WHERE plate = ? AND hash = ?", { json.encode(props), props.engineHealth, props.bodyHealth, props.fuelLevel, plate, props.model }, function(result)
				if result == 0 then print("Failed to save to database") return end
			end)
		end
	end
end
...
RegisterNetEvent(getScript()..":server:preview", function(active, vehicle, plate)
	local src = source
	if active then
		debugPrint("^5Debug^7: ^3Preview: ^2Player^7(^4"..src.."^7)^2 Started previewing^7")
		Previewing[src] = { active = active, vehicle = vehicle, plate = plate }
	else
		debugPrint("^5Debug^7: ^3Preview: ^2Player^7(^4"..src.."^7)^2 Stopped previewing^7")
		Previewing[src] = nil
	end
end)
...
AddEventHandler("playerDropped", function()
	...
	if Previewing[src] then
		debugPrint("^5SQL^7: ^3Preview: ^2Player dropped while previewing^7, ^2resetting vehicle properties^7")
		local properties = {}
		local result = MySQL.query.await("SELECT mods FROM "..vehDatabase.." WHERE plate = ?", { Previewing[src].plate })
		if result[1] then TriggerClientEvent(getScript()..":preview:exploitfix", -1, Previewing[src].vehicle, json.decode(result[1].mods)) end
			...
			for locKey, location in pairs(Locations) do
				if isJobRole(location.job) then
					for index, _ in pairs(location.Stash) do
						...
						registerStash(name, (location.blip and location.blip.label or "").." Mech Stash ".."("..index..")"..(debugMode and " - ["..name.."]" or ""), info.slots or 50, info.maxWeight or 4000000)
