			...
			<div class="col pr05 selection-colw">
				<div class="card h-100">
					<div class="selectionMyInvoices card-body text-center selection-subcard" id="selectionMyInvoices" onclick="selectionMyInvoices()">
						...
						<p class="selection-subcard_title">Factures<br>Personnelles</p>
					</div>
				</div>
			</div>
			...
			if(playerJobIsInBillsList){
				...
				if(!p2p){
					$('#invoicesFirstMenu').html(`
						<button type="button" class="btn btn-blue w-100 ci-option">MÉTIER</button>
					`);
					$('#invoicesSecondMenu').html(`
						<button type="button" id="openBillsListInvoice" class="btn btn-odark w-100 ci-option">MÉTIER</button>
					`);
				}
			}
			else{
