...
local useWeedSeed = function()
    print("Function useWeedSeed démarrée")
    if cache.vehicle then 
        print("Erreur: Le joueur est dans un véhicule")
        return 
    end

    local hasItem = client.hasItems(Config.FemaleSeed, 1)
    print("Vérification item: " .. Config.FemaleSeed .. ", résultat: " .. tostring(hasItem))
    if not hasItem then return end

    if placingSeed then 
        print("Erreur: Déjà en train de placer une graine")
        return 
    end
    placingSeed = true
    seedPlaced = false
    ...
    -- })
    exports['jg-textui']:DrawText(Locales['place_or_cancel'])
    print("TextUI affiché: " .. Locales['place_or_cancel'])

    local hit, entityHit, endCoords, surfaceNormal, materialHash = RayCast(511, 4, rayCastDistance)
    print("Premier RayCast - hit: " .. tostring(hit) .. ", materialHash: " .. tostring(materialHash))
    ...
    print("Objet créé avec le hash: " .. tostring(ModelHash))
    ...
    while not seedPlaced do
        ...
        if IsControlPressed(0, 186) then
            -- lib.hideTextUI()
            print("Annulation avec la touche X")
        ...
        if hit then
            ...
            if IsControlJustPressed(0, 38) then
                print("Touche E pressée pour placer la plante")
                print("MaterialHash: " .. tostring(materialHash))

                if materialHash == nil then
                    print("ERREUR: materialHash est nil!")
                    utils.notify(Locales['notify_title_planting'], "Erreur de surface: materialHash nil", 'error', 3000)
                    Wait(200)
                elseif Config.GroundHashes[materialHash] then
                    print("Surface valide trouvée dans Config.GroundHashes: " .. tostring(materialHash))
                    ...
                    if lib.progressBar({
                        ...
                        print("ProgressBar terminée avec succès, envoi de l'événement CreateNewPlant")
                        TriggerServerEvent('weedplanting:server:CreateNewPlant', endCoords)
                        placingSeed = false
                        ClearPedTasks(ped)
                        return
                    else
                        print("ProgressBar annulée")
                        placingSeed = false
                        ClearPedTasks(ped)
                    ...
                    print("Surface invalide pour planter. MaterialHash: " .. tostring(materialHash))
                    print("GroundHashes disponibles: ")
...
RegisterNetEvent('weedplanting:client:ClearPlant', function(entity)
    local plantId = WeedPlantCache[entity]
    if not plantId then 
        print('^3[ps-weedplanting:client] ^1Erreur: Impossible de trouver l\'ID de la plante pour l\'entité ' .. tostring(entity) .. '^7')
        return 
    end

    print('^3[ps-weedplanting:client] ^5Nettoyage de la plante ID: ' .. tostring(plantId) .. '^7')
    local ped = cache.ped
    ...
    if lib.progressBar({
        ...
        print('^3[ps-weedplanting:client] ^5Envoi de l\'événement server:ClearPlant pour plantId: ' .. tostring(plantId) .. '^7')
        TriggerServerEvent('weedplanting:server:ClearPlant', plantId, false)
        ClearPedTasks(ped)
        utils.notify(Locales['notify_title_planting'], 'Plant successfully cleared', 'success', 3000)
    else
        ClearPedTasks(ped)
        utils.notify(Locales['notify_title_planting'], Locales['canceled'], 'error', 3000)
    end
