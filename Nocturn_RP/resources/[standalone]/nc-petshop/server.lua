...
AddEventHandler('nc-petshop:buyPet', function(petType,price)
	...
	exports.oxmysql:execute('SELECT * FROM pets WHERE owner = ?', {
		...
		if ifOwner ~= nil then
			TriggerClientEvent('QBCore:Notify', _source, 'Vous possédez déjà un animal !', 'error')
			print('Vous possédez déjà un animal !')
		else
			if xPlayer.PlayerData.money.cash > price then
				TriggerClientEvent('QBCore:Notify', _source, 'Vous avez acheté un animal !', 'success')
				print('Vous avez acheté un animal !')
				...
				TriggerClientEvent('QBCore:Notify', _source, 'Vous n\'avez pas assez d\'argent pour cet animal', 'error')
