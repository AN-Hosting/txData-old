			...
			for _, v in pairs(result) do
				...
				if info == "all" or info == "reserves" then
					...
					if Config.UnlimitedFuel then
						ReservesNotBuyable = true
						if Config.FuelDebug then print(
							"Reserves are not buyable, because Config.UnlimitedFuel is set to true.") end
					end
				end
				-- Fuel Price --
...
local function CanAfford(price, purchasetype)
	...
	if purchasetype == "bank" then Money = QBCore.Functions.GetPlayerData().money['bank'] elseif purchasetype == 'cash' then Money =
		QBCore.Functions.GetPlayerData().money['cash'] end
	if Money < price then
		return false
	else
		return true
	...
	CreateThread(function()
		while true do
			...
			if IsPedInAnyVehicle(ped, false) and IsControlPressed(2, 75) and not IsEntityDead(ped) then
				...
				if Config.FuelDebug then if enginerunning then print('Engine is running!') else print(
						'Engine is not running!') end end
				Wait(900)
...
if Config.ShowNearestGasStationOnly then
	...
	RegisterNetEvent('cdn-fuel:client:updatestationlabels', function(location, newLabel)
		...
		if Config.FuelDebug then print("Changing Label for Location #" .. location .. ' to ' .. newLabel) end
		Config.GasStations[location].label = newLabel
		local coords = vector3(Config.GasStations[location].pedcoords.x, Config.GasStations[location].pedcoords.y,
	...
	CreateThread(function()
		...
		for i = 1, #Config.GasStations, 1 do
			local location = i
			gasStationCoords = vector3(Config.GasStations[location].pedcoords.x, Config.GasStations[location].pedcoords
			.y, Config.GasStations[location].pedcoords.z)
			...
			if (fuel.amount + finalfuel) >= 100 then
				...
				if GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice) <= money then
					if Config.FuelDebug then
						...
						fuel.amount ..
						"L of Fuel @ " ..
						FuelPrice .. '/L, Total Cost: ' .. GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice))
					end
				...
				if lib.progressCircle({
					...
					if RefuelingType == nil then
						if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
							...
							if Config.FuelDebug then print(
								"Config.PlayerOwnedGasStationsEnabled == false or Config.UnlimitedFuel == true, this means reserves will not be changed.") end
						end
						if Config.FuelDebug then print(
							"Config.PlayerOwnedGasStationsEnabled == false or Config.UnlimitedFuel == true, this means reserves will not be changed.") end
					end
					StopAnimTask(ped, Config.RefuelAnimationDictionary, Config.RefuelAnimation, 3.0, 3.0, -1, 2, 0, 0, 0,
						0)
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelmenu', function(itemData)
	...
	-- Controleer of de speler in een voertuig zit
	if IsPedInAnyVehicle(playerPed, false) then
		QBCore.Functions.Notify(Lang:t("cannot_refuel_inside"), 'error')
		return
	end

	-- Debugging: Toon itemdata in de console
	if Config.FuelDebug then
		print("Item Data: " .. json.encode(itemData))
	end

	-- Haal het dichtstbijzijnde voertuig op
	local vehicle = GetClosestVehicle()
	local vehicleCoords = GetEntityCoords(vehicle)
	local pedCoords = GetEntityCoords(playerPed)

	-- Controleer of het voertuig te veel schade heeft
	if GetVehicleBodyHealth(vehicle) < 100 then
		QBCore.Functions.Notify(Lang:t("vehicle_is_damaged"), 'error')
		return
	end

	-- Bepaal de hoeveelheid brandstof in de jerrycan
	local jerrycanAmount = itemData.info.gasamount or 0

	-- Menu voor het bijtanken van de jerrycan
	if holdingnozzle then
		local fullTank = jerrycanAmount == Config.JerryCanCap
		local gasString = fullTank and Lang:t("menu_jerry_can_footer_full_gas") or
		Lang:t("menu_jerry_can_footer_refuel_gas")
		...
		-- Controleer of de speler dichtbij het voertuig is
		if #(vehicleCoords - pedCoords) > 2.5 then
			return
		end

		-- Menu voor het bijtanken van het voertuig
		local noGas = jerrycanAmount < 1
		local gasString = noGas and Lang:t("menu_jerry_can_footer_no_gas") or Lang:t("menu_jerry_can_footer_use_gas")
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelvehicle', function(data)
	...
	if NotElectric then
		...
		-- Gebruik qb-input voor het invoeren van de hoeveelheid brandstof
		...
		if refuel then
			...
			if refuelAmount == 0 then
				...
				return
			elseif refuelAmount < 0 then
				QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error')
				return
			end

			if refuelAmount > jerrycanfuelamount then
				QBCore.Functions.Notify(Lang:t("jerry_can_not_enough_fuel"), 'error')
				return
			end

			local refueltimer = Config.RefuelTime * refuelAmount
			if refuelAmount < 10 then
				refueltimer = Config.RefuelTime * 10
			end

			if vehfuel + refuelAmount > 100 then
				QBCore.Functions.Notify(Lang:t("tank_cannot_fit"), 'error')
				return
			end

			-- Creëer en bevestig de jerrycan prop
			local JerrycanProp = CreateObject(joaat('w_am_jerrycan'), 1.0, 1.0, 1.0, true, true, false)
			local lefthand = GetPedBoneIndex(ped, 18905)
			AttachEntityToEntity(JerrycanProp, ped, lefthand, 0.11, 0.0, 0.25, 15.0, 170.0, 90.42, 0, 1, 0, 1, 0, 1)

			-- Gebruik qb-progressbar voor de voortgang
			QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_refueling_vehicle"), refueltimer, false, true, {
				...
				-- Verwijder brandstof uit de jerrycan en update de inventaris
				local JerryCanItemData = data.itemData
				local srcPlayerData = QBCore.Functions.GetPlayerData()
				TriggerServerEvent('cdn-fuel:info', "remove", refuelAmount, srcPlayerData, JerryCanItemData)

				-- Vul de brandstof van het voertuig bij
	...
	if Config.Ox.Input then
		...
		if refuel then
			...
			QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_jerry_can_refuel"), refueltimer, false, true,
				...
				if RefuelingType == nil then
					if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
						TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove", tonumber(refuel.amount),
							ReserveLevels, CurrentLocation)
						if CachedFuelPrice ~= nil then
							TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuel.amount),
								StationBalance, CurrentLocation, CachedFuelPrice)
						else
							TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", tonumber(refuel.amount),
								StationBalance, CurrentLocation, FuelPrice)
...
local function PoliceAlert(coords)
	...
	if chance < Config.SyphonPoliceCallChance then
		...
		elseif Config.SyphonDispatchSystem == "custom" then
			...
			if Config.SyphonDebug then print(
				"There was an attempt to call police but this dispatch system is not supported!") end
		end
	end
end
...
RegisterNetEvent('cdn-syphoning:syphon:menu', function(itemData)
	...
	if Config.ElectricVehicleCharging then
		...
		if Config.ElectricVehicles[vehiclename] and Config.ElectricVehicles[vehiclename].isElectric then
			NotElectric = false
			if Config.SyphonDebug then print("^2" ..
				current ..
				"^5 has been found. It ^2matches ^5the Player's Vehicle: ^2" ..
				vehiclename .. ". ^5This means syphoning will not be allowed.") end
						...
						if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
							...
							QBCore.Functions.Progressbar('syphon_gas', Lang:t("prog_syphoning"), syphontimer, false, true,
								...
								if GetFuel(vehicle) >= tonumber(syphon.amount) then
									...
									local syphonData = data.itemData
									local srcPlayerData = QBCore.Functions.GetPlayerData()
									TriggerServerEvent('cdn-fuel:info', "add", tonumber(syphon.amount), srcPlayerData,
										syphonData)
									StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
								else
									QBCore.Functions.Notify(Lang:t("menu_syphon_vehicle_empty"), 'error')
...
RegisterNetEvent('cdn-fuel:client:grabnozzle:special', function()
	...
	if Config.PumpHose then
		...
		if Config.FuelDebug then
			print("Hose Properties:")
			print(Rope, pump, SpecialFuelNozzleObj, pumpCoords.x, pumpCoords.y, pumpCoords.z, nozzlePos.x, nozzlePos.y,
				nozzlePos.z, length)
	...
	CreateThread(function()
		while HoldingSpecialNozzle do
			...
			if dist > Config.AirAndWaterVehicleFueling['nozzle_length'] or IsPedInAnyVehicle(ped, false) then
				...
				if Config.FuelNozzleExplosion then
					AddExplosion(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z, 'EXP_TAG_PROPANE',
						1.0, true, false, 5.0)
		...
		if LocalPlayer.state['isLoggedIn'] then
			for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
								...
								while PlayerInSpecialFuelZone do
									...
									if IsControlJustReleased(0, Config.AirAndWaterVehicleFueling['refuel_button']) --[[ Control in Config ]] then
										...
										elseif IsPedInAnyVehicle(PlayerPedId(), true) then
											...
											if Config.FuelDebug then print(
												"Attempting to Open Fuel menu for special vehicles.") end
				...
				if currentLocation['prop'] then
					...
					if Config.FuelDebug then print("Location #" ..
						i ..
						" for Special Fueling Zones (Air and Sea) doesn't have a prop set up, so players cannot fuel here.") end
				end
...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
		...
		if currentLocation['prop'] then
			...
			if Config.FuelDebug then print("Location #" ..
				i .. " for Special Fueling Zones (Air and Sea) doesn't have a prop set up, so players cannot fuel here.") end
		end
	...
	if Config.TargetResource == 'ox_target' then
		...
		local modelOptions = {
			...
			[4] = {
				...
				canInteract = function()
					if Config.FuelDebug then print("Is Player In Special Fuel Zone?: " ..
						tostring(PlayerInSpecialFuelZone)) end
