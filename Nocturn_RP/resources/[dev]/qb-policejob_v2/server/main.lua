...
QBCore.Commands.Add("objectmenu", Lang:t("commands.place_object"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
        TriggerClientEvent('police:client:OpenObjectMenu', src)
    end
end)
...
QBCore.Functions.CreateCallback('police:server:PayForVehicle', function(source, cb, price, vehicle)
    ...
    if Config.OwnedPoliceCars then plate = GeneratePlate() else plate = Lang:t('info.police_plate')..tostring(math.random(1000, 9999)) end
...
RegisterNetEvent('police:server:SearchPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        local sPlayercid = QBCore.Functions.GetPlayer(tonumber(player)).PlayerData.citizenid
...
RegisterNetEvent('police:server:spawnObject', function(object, loc, heading)
    local src = source
    local objectId = CreateObjectId()
    local type = object
    Objects[objectId] = type
    TriggerClientEvent("police:client:spawnObject", src, objectId, type, loc, heading)
end)
